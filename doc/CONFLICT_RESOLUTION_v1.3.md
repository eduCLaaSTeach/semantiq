...
| Application state | No app scaffold yet | Phase 00 foundation | Phase 00 starts with repository assessment and scaffolding. SUPERSEDED BY FACT on 22 Aug 2026: the application is scaffolded and deployed. `composer.json` and `package.json` exist, the suite runs, and the design system, Entra ID sign-in and application shell are live. The original caution against claiming a green build before the scaffold existed no longer applies; the standing rule is the unchanged one, that a result is only reported after it has actually been run. |
