...
- **Ask, don't assume.** Do not assume stack, hosting, source control, CI/CD, runtime, deployment, architecture, validation commands, security model, or intent. Ask until requirements and success criteria are clear. Many rounds of questions are fine; urgency is not a reason to guess. (`hard-stops.md`, `project-intake.md`, `fresh-project-gateway.md`. **OSI-4 adds the evidence discipline: verify before stating, cite the source (file path and symbol, branch and commit, or the command whose output you read), label every unverified claim as unverified in the same sentence, and never substitute volume of plausible detail for a fact you did not check. "I have not verified that" is a complete answer.** (`owner-standing-instructions.md`.)
...
- **Git is always allowed; deploy/migrate is not.** All Git/GitHub actions (commit, push, branch, PR, merge, tag, even to PROD) need no per-action confirmation - but state source/target/files/command first. Running an actual production deploy or database migration is a separate action that always needs explicit approval. (`enterprise-governance.md`, `git-branching-release.md`, `deployment.md`. **Owner standing instruction OSI-1 replaces the PR hand-off default: after any push, create the Pull Request automatically and return its URL. Merging, promotion, and deploy stay with the owner.** (`owner-standing-instructions.md`.)
...
- **Owner standing instructions (always in force).** Five instructions issued by the project owner, binding in every session. **OSI-1** automate the Pull Request and return its URL on any Git task. **OSI-2** name the security considerations at design time for every feature - trust boundary, server-side authorisation, data classification, secret handling, input handling, fail-closed behaviour, auditability - and pass gate REV over the real diff before claiming done. **OSI-3** end every completed task with completion detail, then active PR monitoring, then the owner's ordered step-by-step guide of only what Claude cannot do. **OSI-4** verify, never assume, and never pad. **OSI-5** on a feature the owner confirms working, offer all three living documents together - feature list, data dictionary, code dictionary - through the `knowledge-base.md` approval gate, which OSI-5 does not weaken. (Gate OSI.)
...
| **OSI** | does any Git/PR action, delivers or completes a feature, writes a completion report, or updates the knowledge base | `.claude/rules/owner-standing-instructions.md` |
...
- **OSI-3 shape (always, on any completed task):** *Completion detail* · *PR state and monitoring* · *Your step-by-step guide* - the last being only the actions that genuinely require the owner, or an explicit "nothing required on your side".
